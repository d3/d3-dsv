...
tape("csvFormat(array) converts dates to ISO 8601", function(test) {
  ...
  test.deepEqual(dsv.csvFormat([{date: new Date(2018, 0, 1)}]), "date\n" + new Date(2018, 0, 1).toISOString().replace(/(T00:00:00.000Z|:00.000)/,""));
  test.end();
});
...
tape("csvFormatRows(array) converts dates to ISO 8601", function(test) {
  ...
  test.deepEqual(dsv.csvFormatRows([[new Date(2018, 0, 1)]]), new Date(2018, 0, 1).toISOString().replace(/(T00:00:00.000Z|:00.000)/,""));
  test.end();
});
