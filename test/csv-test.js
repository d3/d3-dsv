...
tape("csv.parse(string) returns the expected objects", function(test) {
  ...
  test.deepEqual(dsv.csv.parse(fs.readFileSync("test/data/sample.csv", "utf-8")), [{Hello: "42", World: "\"fish\""}]);
  test.end();
});
...
tape("csv.parse(string) observes Unix, Mac and DOS newlines", function(test) {
  ...
  test.deepEqual(dsv.csv.parse("a,b,c\r1,2,3\r4,5,\"6\"\r7,8,9"), [{a: "1", b: "2", c: "3"}, {a: "4", b: "5", c: "6"}, {a: "7", b: "8", c: "9"}]);
  test.deepEqual(dsv.csv.parse("a,b,c\r\n1,2,3\r\n4,5,\"6\"\r\n7,8,9"), [{a: "1", b: "2", c: "3"}, {a: "4", b: "5", c: "6"}, {a: "7", b: "8", c: "9"}]);
  test.end();
});

tape("csv.parse(string, convert) returns the expected converted objects", function(test) {
  function convert(row) { row.Hello = -row.Hello; return row; }
  test.deepEqual(dsv.csv.parse(fs.readFileSync("test/data/sample.csv", "utf-8"), convert), [{Hello: -42, World: "\"fish\""}]);
  test.deepEqual(dsv.csv.parse("a,b,c\n1,2,3\n", function(row) { return row; }), [{a: "1", b: "2", c: "3"}]);
  test.end();
});

tape("csv.parse(string, convert) skips rows if convert returns null or undefined", function(test) {
...
tape("csv.parse(string, convert) invokes convert for every row in order", function(test) {
  var rows = [];
...
tape("csv.parseRows(string, convert) returns the expected converted array of array of string", function(test) {
  function convert(row, i) { if (i) row[0] = -row[0]; return row; }
  test.deepEqual(dsv.csv.parseRows(fs.readFileSync("test/data/sample.csv", "utf-8"), convert), [["Hello", "World"], [-42, "\"fish\""]]);
  test.deepEqual(dsv.csv.parseRows("a,b,c\n1,2,3\n", function(row) { return row; }), [["a", "b", "c"], ["1", "2", "3"]]);
  test.end();
});

tape("csv.parseRows(string, convert) skips rows if convert returns null or undefined", function(test) {
  function convert(row, i) { return [row, null, undefined, false][i]; }
...
tape("csv.parseRows(string, convert) invokes convert for every row in order", function(test) {
  var rows = [];
  dsv.csv.parseRows("a\n1\n2\n3\n4", function(d, i) { rows.push({d: d, i: i}); });
