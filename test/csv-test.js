...
it("csvParse(string) returns the expected objects", () => {
  assert.deepStrictEqual(d3.csvParse("a,b,c\n1,2,3\n"), table([{a: "1", b: "2", c: "3"}], ["a", "b", "c"]));
  assert.deepStrictEqual(d3.csvParse(readFileSync("test/data/sample.csv", "utf-8")), table([{Hello: "42", World: "\"fish\""}], ["Hello", "World"]));
});
...
it("csvParse(string) parses quoted values with quotes", () => {
  assert.deepStrictEqual(d3.csvParse("a\n\"\"\"hello\"\"\""), table([{a: "\"hello\""}], ["a"]));
});
...
it("csvParse(string) observes Unix, Mac and DOS newlines", () => {
  ...
  assert.deepStrictEqual(d3.csvParse("a,b,c\r1,2,3\r4,5,\"6\"\r7,8,9"), table([{a: "1", b: "2", c: "3"}, {a: "4", b: "5", c: "6"}, {a: "7", b: "8", c: "9"}], ["a", "b", "c"]));
  assert.deepStrictEqual(d3.csvParse("a,b,c\r\n1,2,3\r\n4,5,\"6\"\r\n7,8,9"), table([{a: "1", b: "2", c: "3"}, {a: "4", b: "5", c: "6"}, {a: "7", b: "8", c: "9"}], ["a", "b", "c"]));
});

it("csvParse(string) returns columns in the input order", () => {
  assert.deepStrictEqual(d3.csvParse("a,b,c\n").columns, ["a", "b", "c"]);
  assert.deepStrictEqual(d3.csvParse("a,c,b\n").columns, ["a", "c", "b"]);
  assert.deepStrictEqual(d3.csvParse("a,0,1\n").columns, ["a", "0", "1"]);
  assert.deepStrictEqual(d3.csvParse("1,0,a\n").columns, ["1", "0", "a"]);
});

it("csvParse(string) passes the csv-spectrum test suite", () => {
  spectrum(function(error, samples) {
    samples.forEach(function(sample) {
      const actual = d3.csvParse(sample.csv.toString()),
...
it("csvParse(string, row) returns the expected converted objects", () => {
  ...
  assert.deepStrictEqual(d3.csvParse(readFileSync("test/data/sample.csv", "utf-8"), row), table([{Hello: -42, World: "\"fish\""}], ["Hello", "World"]));
...
it("csvParse(string, row) skips rows if row returns null or undefined", () => {
  ...
  assert.deepStrictEqual(d3.csvParse("field\n42\n\n\n\n", row), table([{field: "42"}, false], ["field"]));
...
it("csvParseRows(string) ignores a blank last line", () => {
  assert.deepStrictEqual(d3.csvParseRows("1,2,3\n"), [["1", "2", "3"]]);
});
...
it("csvParseRows(string) can return rows of varying length", () => {
  assert.deepStrictEqual(d3.csvParseRows("1\n1,2\n1,2,3"), [["1"], ["1", "2"], ["1", "2", "3"]]);
});
...
it("csvParseRows(string) parses quoted values", () => {
  assert.deepStrictEqual(d3.csvParseRows("\"1\",2,3\n"), [["1", "2", "3"]]);
  assert.deepStrictEqual(d3.csvParseRows("\"hello\""), [["hello"]]);
});
...
it("csvParseRows(string) parses quoted values with newlines", () => {
  assert.deepStrictEqual(d3.csvParseRows("\"new\nline\""), [["new\nline"]]);
  assert.deepStrictEqual(d3.csvParseRows("\"new\rline\""), [["new\rline"]]);
  assert.deepStrictEqual(d3.csvParseRows("\"new\r\nline\""), [["new\r\nline"]]);
});

it("csvParseRows(string) parses Unix, Mac and DOS newlines", () => {
  assert.deepStrictEqual(d3.csvParseRows("a,b,c\n1,2,3\n4,5,\"6\"\n7,8,9"), [["a", "b", "c"], ["1", "2", "3"], ["4", "5", "6"], ["7", "8", "9"]]);
  assert.deepStrictEqual(d3.csvParseRows("a,b,c\r1,2,3\r4,5,\"6\"\r7,8,9"), [["a", "b", "c"], ["1", "2", "3"], ["4", "5", "6"], ["7", "8", "9"]]);
  assert.deepStrictEqual(d3.csvParseRows("a,b,c\r\n1,2,3\r\n4,5,\"6\"\r\n7,8,9"), [["a", "b", "c"], ["1", "2", "3"], ["4", "5", "6"], ["7", "8", "9"]]);
});

it("csvParseRows(\"\") returns the empty array", () => {
  assert.deepStrictEqual(d3.csvParseRows(""), []);
});

it("csvParseRows(\"\n\") returns an array of one empty string", () => {
  assert.deepStrictEqual(d3.csvParseRows("\n"), [[""]]);
  assert.deepStrictEqual(d3.csvParseRows("\r"), [[""]]);
  assert.deepStrictEqual(d3.csvParseRows("\r\n"), [[""]]);
});
...
it("csvParseRows(string, row) returns the expected converted array of array of string", () => {
  ...
  assert.deepStrictEqual(d3.csvParseRows(readFileSync("test/data/sample.csv", "utf-8"), row), [["Hello", "World"], [-42, "\"fish\""]]);
  assert.deepStrictEqual(d3.csvParseRows("a,b,c\n1,2,3\n", function(d) { return d; }), [["a", "b", "c"], ["1", "2", "3"]]);
});

it("csvParseRows(string, row) skips rows if row returns null or undefined", () => {
  function row(d, i) { return [d, null, undefined, false][i]; }
  assert.deepStrictEqual(d3.csvParseRows("field\n42\n\n\n", row), [["field"], false]);
...
it("csvFormat(array) takes an array of objects as input", () => {
  assert.deepStrictEqual(d3.csvFormat([{a: 1, b: 2, c: 3}]), "a,b,c\n1,2,3");
});

it("csvFormat(array) converts dates to ISO 8601", () => {
  assert.deepStrictEqual(d3.csvFormat([{date: new Date(Date.UTC(2018, 0, 1))}]), "date\n2018-01-01");
  assert.deepStrictEqual(d3.csvFormat([{date: new Date(2018, 0, 1)}]), "date\n2018-01-01T08:00Z");
});

it("csvFormat(array) escapes field names and values containing delimiters", () => {
  assert.deepStrictEqual(d3.csvFormat([{"foo,bar": true}]), "\"foo,bar\"\ntrue");
  assert.deepStrictEqual(d3.csvFormat([{field: "foo,bar"}]), "field\n\"foo,bar\"");
});

it("csvFormat(array) computes the union of all fields", () => {
  assert.deepStrictEqual(d3.csvFormat([{a: 1}, {a: 1, b: 2}, {a: 1, b: 2, c: 3}, {b: 1, c: 2}, {c: 1}]), "a,b,c\n1,,\n1,2,\n1,2,3\n,1,2\n,,1");
});

it("csvFormat(array) orders fields by first-seen", () => {
  assert.deepStrictEqual(d3.csvFormat([{a: 1, b: 2}, {c: 3, b: 4}, {c: 5, a: 1, b: 2}]), "a,b,c\n1,2,\n,4,3\n1,2,5");
});

it("csvFormat(array, columns) observes the specified array of column names", () => {
  assert.deepStrictEqual(d3.csvFormat([{a: 1, b: 2, c: 3}], ["c", "b", "a"]), "c,b,a\n3,2,1");
  assert.deepStrictEqual(d3.csvFormat([{a: 1, b: 2, c: 3}], ["c", "a"]), "c,a\n3,1");
  assert.deepStrictEqual(d3.csvFormat([{a: 1, b: 2, c: 3}], []), "\n");
  assert.deepStrictEqual(d3.csvFormat([{a: 1, b: 2, c: 3}], ["d"]), "d\n");
});

it("csvFormat(array, columns) coerces column names to strings", () => {
  assert.deepStrictEqual(d3.csvFormat([{a: 1, b: 2, "\"fish\"": 3}], [{toString: function() { return "\"fish\""; }}]), "\"\"\"fish\"\"\"\n3");
  assert.deepStrictEqual(d3.csvFormat([{a: 1, b: 2, c: 3}], ["a", null, "b", undefined, "c"]), "a,,b,,c\n1,,2,,3");
});

it("csvFormat(array, columns) coerces field values to strings", () => {
  assert.deepStrictEqual(d3.csvFormat([{a: {toString: function() { return "\"fish\""; }}}]), "a\n\"\"\"fish\"\"\"");
  assert.deepStrictEqual(d3.csvFormat([{a: null, b: undefined, c: 3}]), "a,b,c\n,,3");
});

it("csvFormatBody(array) omits the header row", () => {
  assert.deepStrictEqual(d3.csvFormatBody([{a: 1, b: 2}, {c: 3, b: 4}, {c: 5, a: 1, b: 2}]), "1,2,\n,4,3\n1,2,5");
});

it("csvFormatBody(array, columns) omits the header row", () => {
  assert.deepStrictEqual(d3.csvFormatBody([{a: 1, b: 2}, {c: 3, b: 4}, {c: 5, a: 1, b: 2}], ["a", "b"]), "1,2\n,4\n1,2");
});

it("csvFormatRows(array) takes an array of array of string as input", () => {
  assert.deepStrictEqual(d3.csvFormatRows([["a", "b", "c"], ["1", "2", "3"]]), "a,b,c\n1,2,3");
});

it("csvFormatRows(array) separates lines using Unix newline", () => {
  assert.deepStrictEqual(d3.csvFormatRows([[], []]), "\n");
});

it("csvFormatRows(array) converts dates to ISO 8601", () => {
  assert.deepStrictEqual(d3.csvFormatRows([[new Date(Date.UTC(2018, 0, 1))]]), "2018-01-01");
  assert.deepStrictEqual(d3.csvFormatRows([[new Date(2018, 0, 1)]]), "2018-01-01T08:00Z");
});
...
it("csvFormatRows(array) does not quote simple values", () => {
  assert.deepStrictEqual(d3.csvFormatRows([["a"], [1]]), "a\n1");
});

it("csvFormatRows(array) escapes double quotes", () => {
  assert.deepStrictEqual(d3.csvFormatRows([["\"fish\""]]), "\"\"\"fish\"\"\"");
});

it("csvFormatRows(array) escapes Unix newlines", () => {
  assert.deepStrictEqual(d3.csvFormatRows([["new\nline"]]), "\"new\nline\"");
});

it("csvFormatRows(array) escapes Windows newlines", () => {
  assert.deepStrictEqual(d3.csvFormatRows([["new\rline"]]), "\"new\rline\"");
});

it("csvFormatRows(array) escapes values containing delimiters", () => {
  assert.deepStrictEqual(d3.csvFormatRows([["oxford,comma"]]), "\"oxford,comma\"");
});

it("csvFormatRow(array) takes a single array of string as input", () => {
  assert.deepStrictEqual(d3.csvFormatRow(["a", "b", "c"]), "a,b,c");
});
