...
tape("dsv(\"|\").parse(string, identity) returns the expected objects", function(test) {
  test.deepEqual(psv.parse("a|b|c\n1|2|3\n", identity), table([{a: "1", b: "2", c: "3"}], ["a", "b", "c"]));
  test.deepEqual(psv.parse(fs.readFileSync("test/data/sample.psv", "utf-8"), identity), table([{Hello: "42", World: "\"fish\""}], ["Hello", "World"]));
  test.end();
});

tape("dsv(\"|\").parse(string, identity) does not strip whitespace", function(test) {
  test.deepEqual(psv.parse("a|b|c\n 1| 2|3\n", identity), table([{a: " 1", b: " 2", c: "3"}], ["a", "b", "c"]));
  test.end();
});

tape("dsv(\"|\").parse(string, identity) parses quoted values", function(test) {
  test.deepEqual(psv.parse("a|b|c\n\"1\"|2|3", identity), table([{a: "1", b: "2", c: "3"}], ["a", "b", "c"]));
  test.deepEqual(psv.parse("a|b|c\n\"1\"|2|3\n", identity), table([{a: "1", b: "2", c: "3"}], ["a", "b", "c"]));
  test.end();
});

tape("dsv(\"|\").parse(string, identity) parses quoted values with quotes", function(test) {
  test.deepEqual(psv.parse("a\n\"\"\"hello\"\"\"", identity), table([{a: "\"hello\""}], ["a"]));
  test.end();
});

tape("dsv(\"|\").parse(string, identity) parses quoted values with newlines", function(test) {
...
tape("dsv(\"|\").parse(string, identity) observes Unix, Mac and DOS newlines", function(test) {
  test.deepEqual(psv.parse("a|b|c\n1|2|3\n4|5|\"6\"\n7|8|9", identity), table([{a: "1", b: "2", c: "3"}, {a: "4", b: "5", c: "6"}, {a: "7", b: "8", c: "9"}], ["a", "b", "c"]));
  test.deepEqual(psv.parse("a|b|c\r1|2|3\r4|5|\"6\"\r7|8|9", identity), table([{a: "1", b: "2", c: "3"}, {a: "4", b: "5", c: "6"}, {a: "7", b: "8", c: "9"}], ["a", "b", "c"]));
  test.deepEqual(psv.parse("a|b|c\r\n1|2|3\r\n4|5|\"6\"\r\n7|8|9", identity), table([{a: "1", b: "2", c: "3"}, {a: "4", b: "5", c: "6"}, {a: "7", b: "8", c: "9"}], ["a", "b", "c"]));
  test.end();
});
...
tape("dsv(\"|\").parse(string, row) returns the expected converted objects", function(test) {
  ...
  test.deepEqual(psv.parse(fs.readFileSync("test/data/sample.psv", "utf-8", identity), row), table([{Hello: -42, World: "\"fish\""}], ["Hello", "World"]));
...
tape("dsv(\"|\").parseRows(string, identity) returns the expected array of array of string", function(test) {
  test.deepEqual(psv.parseRows("a|b|c\n", identity), [["a", "b", "c"]]);
  test.end();
});

tape("dsv(\"|\").parseRows(string, identity) parses quoted values", function(test) {
  test.deepEqual(psv.parseRows("\"1\"|2|3\n", identity), [["1", "2", "3"]]);
  test.deepEqual(psv.parseRows("\"hello\"", identity), [["hello"]]);
  test.end();
});

tape("dsv(\"|\").parseRows(string, identity) parses quoted values with quotes", function(test) {
  test.deepEqual(psv.parseRows("\"\"\"hello\"\"\"", identity), [["\"hello\""]]);
  test.end();
});

tape("dsv(\"|\").parseRows(string, identity) parses quoted values with newlines", function(test) {
  test.deepEqual(psv.parseRows("\"new\nline\"", identity), [["new\nline"]]);
  test.deepEqual(psv.parseRows("\"new\rline\"", identity), [["new\rline"]]);
  test.deepEqual(psv.parseRows("\"new\r\nline\"", identity), [["new\r\nline"]]);
  test.end();
});

tape("dsv(\"|\").parseRows(string, identity) parses Unix, Mac and DOS newlines", function(test) {
  test.deepEqual(psv.parseRows("a|b|c\n1|2|3\n4|5|\"6\"\n7|8|9", identity), [["a", "b", "c"], ["1", "2", "3"], ["4", "5", "6"], ["7", "8", "9"]]);
  test.deepEqual(psv.parseRows("a|b|c\r1|2|3\r4|5|\"6\"\r7|8|9", identity), [["a", "b", "c"], ["1", "2", "3"], ["4", "5", "6"], ["7", "8", "9"]]);
  test.deepEqual(psv.parseRows("a|b|c\r\n1|2|3\r\n4|5|\"6\"\r\n7|8|9", identity), [["a", "b", "c"], ["1", "2", "3"], ["4", "5", "6"], ["7", "8", "9"]]);
  test.end();
});
...
tape("dsv(\"|\").parseRows(string, row) returns the expected converted array of array of string", function(test) {
  ...
  test.deepEqual(psv.parseRows(fs.readFileSync("test/data/sample.psv", "utf-8", identity), row), [["Hello", "World"], [-42, "\"fish\""]]);
