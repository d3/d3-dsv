...
it("tsvParse(string) returns the expected objects", () => {
  assert.deepStrictEqual(d3.tsvParse("a\tb\tc\n1\t2\t3\n"), table([{a: "1", b: "2", c: "3"}], ["a", "b", "c"]));
  assert.deepStrictEqual(d3.tsvParse(readFileSync("test/data/sample.tsv", "utf-8")), table([{Hello: "42", World: "\"fish\""}], ["Hello", "World"]));
});
...
it("tsvParse(string) parses quoted values with quotes", () => {
  assert.deepStrictEqual(d3.tsvParse("a\n\"\"\"hello\"\"\""), table([{a: "\"hello\""}], ["a"]));
});
...
it("tsvParse(string) observes Unix, Mac and DOS newlines", () => {
  ...
  assert.deepStrictEqual(d3.tsvParse("a\tb\tc\r1\t2\t3\r4\t5\t\"6\"\r7\t8\t9"), table([{a: "1", b: "2", c: "3"}, {a: "4", b: "5", c: "6"}, {a: "7", b: "8", c: "9"}], ["a", "b", "c"]));
  assert.deepStrictEqual(d3.tsvParse("a\tb\tc\r\n1\t2\t3\r\n4\t5\t\"6\"\r\n7\t8\t9"), table([{a: "1", b: "2", c: "3"}, {a: "4", b: "5", c: "6"}, {a: "7", b: "8", c: "9"}], ["a", "b", "c"]));
});

it("tsvParse(string, row) returns the expected converted objects", () => {
  function row(d) { d.Hello = -d.Hello; return d; }
  assert.deepStrictEqual(d3.tsvParse(readFileSync("test/data/sample.tsv", "utf-8"), row), table([{Hello: -42, World: "\"fish\""}], ["Hello", "World"]));
...
it("tsvParse(string, row) skips rows if row returns null or undefined", () => {
  ...
  assert.deepStrictEqual(d3.tsvParse("field\n42\n\n\n\n", row), table([{field: "42"}, false], ["field"]));
...
it("tsvParse(string) accepts numbers as names", () => {
  assert.deepStrictEqual(d3.tsvParse("11\t22\t33\n\"a\"\tb\t0"), table([{11: "a", 22: "b", 33: "0"}], ["11", "22", "33"]));
});

it("tsvParseRows(string) returns the expected array of array of string", () => {
  assert.deepStrictEqual(d3.tsvParseRows("a\tb\tc\n"), [["a", "b", "c"]]);
});

it("tsvParseRows(string) parses quoted values", () => {
  assert.deepStrictEqual(d3.tsvParseRows("\"1\"\t2\t3\n"), [["1", "2", "3"]]);
  assert.deepStrictEqual(d3.tsvParseRows("\"hello\""), [["hello"]]);
});
...
it("tsvParseRows(string) parses quoted values with newlines", () => {
  assert.deepStrictEqual(d3.tsvParseRows("\"new\nline\""), [["new\nline"]]);
  assert.deepStrictEqual(d3.tsvParseRows("\"new\rline\""), [["new\rline"]]);
  assert.deepStrictEqual(d3.tsvParseRows("\"new\r\nline\""), [["new\r\nline"]]);
});

it("tsvParseRows(string) parses Unix, Mac and DOS newlines", () => {
  assert.deepStrictEqual(d3.tsvParseRows("a\tb\tc\n1\t2\t3\n4\t5\t\"6\"\n7\t8\t9"), [["a", "b", "c"], ["1", "2", "3"], ["4", "5", "6"], ["7", "8", "9"]]);
  assert.deepStrictEqual(d3.tsvParseRows("a\tb\tc\r1\t2\t3\r4\t5\t\"6\"\r7\t8\t9"), [["a", "b", "c"], ["1", "2", "3"], ["4", "5", "6"], ["7", "8", "9"]]);
  assert.deepStrictEqual(d3.tsvParseRows("a\tb\tc\r\n1\t2\t3\r\n4\t5\t\"6\"\r\n7\t8\t9"), [["a", "b", "c"], ["1", "2", "3"], ["4", "5", "6"], ["7", "8", "9"]]);
});

it("tsvParseRows(string, row) returns the expected converted array of array of string", () => {
  function row(d, i) { if (i) d[0] = -d[0]; return d; }
  assert.deepStrictEqual(d3.tsvParseRows(readFileSync("test/data/sample.tsv", "utf-8"), row), [["Hello", "World"], [-42, "\"fish\""]]);
  assert.deepStrictEqual(d3.tsvParseRows("a\tb\tc\n1\t2\t3\n", function(d) { return d; }), [["a", "b", "c"], ["1", "2", "3"]]);
});

it("tsvParseRows(string, row) skips rows if row returns null or undefined", () => {
  function row(d, i) { return [d, null, undefined, false][i]; }
  assert.deepStrictEqual(d3.tsvParseRows("field\n42\n\n\n", row), [["field"], false]);
...
it("tsvFormat(array) takes an array of objects as input", () => {
  assert.deepStrictEqual(d3.tsvFormat([{a: 1, b: 2, c: 3}]), "a\tb\tc\n1\t2\t3");
});

it("tsvFormat(array) escapes field names and values containing delimiters", () => {
  assert.deepStrictEqual(d3.tsvFormat([{"foo\tbar": true}]), "\"foo\tbar\"\ntrue");
  assert.deepStrictEqual(d3.tsvFormat([{field: "foo\tbar"}]), "field\n\"foo\tbar\"");
});

it("tsvFormat(array) computes the union of all fields", () => {
  assert.deepStrictEqual(d3.tsvFormat([{a: 1}, {a: 1, b: 2}, {a: 1, b: 2, c: 3}, {b: 1, c: 2}, {c: 1}]), "a\tb\tc\n1\t\t\n1\t2\t\n1\t2\t3\n\t1\t2\n\t\t1");
});

it("tsvFormat(array) orders fields by first-seen", () => {
  assert.deepStrictEqual(d3.tsvFormat([{a: 1, b: 2}, {c: 3, b: 4}, {c: 5, a: 1, b: 2}]), "a\tb\tc\n1\t2\t\n\t4\t3\n1\t2\t5");
});

it("tsvFormat(array, columns) observes the specified array of column names", () => {
  assert.deepStrictEqual(d3.tsvFormat([{a: 1, b: 2, c: 3}], ["c", "b", "a"]), "c\tb\ta\n3\t2\t1");
  assert.deepStrictEqual(d3.tsvFormat([{a: 1, b: 2, c: 3}], ["c", "a"]), "c\ta\n3\t1");
  assert.deepStrictEqual(d3.tsvFormat([{a: 1, b: 2, c: 3}], []), "\n");
  assert.deepStrictEqual(d3.tsvFormat([{a: 1, b: 2, c: 3}], ["d"]), "d\n");
});

it("tsvFormatRows(array) takes an array of array of string as input", () => {
  assert.deepStrictEqual(d3.tsvFormatRows([["a", "b", "c"], ["1", "2", "3"]]), "a\tb\tc\n1\t2\t3");
});

it("tsvFormatRows(array) separates lines using Unix newline", () => {
  assert.deepStrictEqual(d3.tsvFormatRows([[], []]), "\n");
});
...
it("tsvFormatRows(array) does not quote simple values", () => {
  assert.deepStrictEqual(d3.tsvFormatRows([["a"], [1]]), "a\n1");
});

it("tsvFormatRows(array) escapes double quotes", () => {
  assert.deepStrictEqual(d3.tsvFormatRows([["\"fish\""]]), "\"\"\"fish\"\"\"");
});

it("tsvFormatRows(array) escapes Unix newlines", () => {
  assert.deepStrictEqual(d3.tsvFormatRows([["new\nline"]]), "\"new\nline\"");
});

it("tsvFormatRows(array) escapes Windows newlines", () => {
  assert.deepStrictEqual(d3.tsvFormatRows([["new\rline"]]), "\"new\rline\"");
});

it("tsvFormatRows(array) escapes values containing delimiters", () => {
  assert.deepStrictEqual(d3.tsvFormatRows([["oxford\ttab"]]), "\"oxford\ttab\"");
});

it("tsvFormatRow(array) takes a single array of string as input", () => {
  assert.deepStrictEqual(d3.tsvFormatRow(["a", "b", "c"]), "a\tb\tc");
});
